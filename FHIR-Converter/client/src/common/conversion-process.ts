...
async function convertSaveFileShowDifferentialView(activeDataPath: string, activeTemplatePath: string, resultFolder: string, templateFolder: string) {
    ...
    let dataDoc = await getDocText(activeDataPath);

    let msg = globals.coverterEngineHandler.getEngine(DataType.hl7v2).convert(dataDoc, utils.getTemplateNameWithoutExt(templateName), templateFolder, resultFolder); // get the data type from configuration later
    ...
    // show result
    let resultEditor = await showResultEditor(resultFileName);
